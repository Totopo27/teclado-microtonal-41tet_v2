...
function getFrequency(value) {
  ...
  const adjustedValue = value + (currentOctave * 41);
  if (adjustedValue >= 0 && adjustedValue < arrayT41.length) {
    return arrayT41[adjustedValue];
  }
  return null;
}
...
function playNote(config) {
  ...
  const adjustedValue = config.value + (currentOctave * 41);
